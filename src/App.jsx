...
function useTheme() {
  const [theme, setTheme] = useState(() => {
    ...
    return null; // null = follow system preference
  });

  useEffect(() => {
    if (theme === "light" || theme === "dark") {
      document.documentElement.setAttribute("data-theme", theme);
      try { localStorage.setItem("em-theme", theme); } catch (e) { /* ignore */ }
    } else {
      document.documentElement.removeAttribute("data-theme");
      try { localStorage.removeItem("em-theme"); } catch (e) { /* ignore */ }
    }
  }, [theme]);
...
function ThemeToggle({ theme, onChange, t }) {
  ...
  const stateLabel =
    theme === "light" ? t.meta.themeLight :
    theme === "dark" ? t.meta.themeDark :
    t.meta.themeAuto;
  const ariaLabel = `${t.meta.themeToggleLabel}: ${stateLabel}`;
...
function TopBar({ t, lang, onLangChange, theme, onThemeChange }) {
...
function OpenSource({ t }) {
  ...
  return (
    <section className="section" id="opensource" data-screen-label="Open Source">
      <div className="container">
        <p className="kicker reveal">{o.kicker}</p>
        <div className="section-header">
          <h2 className="section-title reveal reveal-delay-1">{o.title}</h2>
          <p className="opensource-lead reveal reveal-delay-2">{o.lead}</p>
        </div>
        ...
        <div className="repo-list">
          {o.repos.map((r, i) => (
            <a
              key={r.name}
              className={`repo-card reveal reveal-delay-${Math.min(i + 1, 4)}`}
              href={r.href}
              target="_blank"
              rel="noreferrer"
              aria-label={`${r.name} — ${o.cta}`}
            >
              ...
              <p className="repo-description">{r.description}</p>
              <div className="repo-stack">
                {r.stack.map((s, j) => (
                  <span key={j} className="chip">{s}</span>
                ))}
              </div>
              <div className="repo-meta">{r.meta}</div>
            </a>
          ))}
        </div>

        <div className="opensource-footer reveal">
          <a href={o.ctaAllHref} target="_blank" rel="noreferrer">{o.ctaAll}</a>
        </div>
      </div>
    </section>
  );
    ...
    <div className="modal-backdrop" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby={`modal-title-${project.id}`}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {project.diagram && (
            <figure className="modal-diagram" role="img" aria-label={m.diagramAlt || m.diagram}>
              <div
                className="modal-diagram-svg"
